...
With both predictors, residuals appear randomly scattered around the 0
line.
...
model\_fit = lm(bwt \~ gaweeks, data = baby\_df)
...
NEED TO CHANGE TO 5000

``` r
# Run the bootstrap to calculate mean r - square
weather_df %>% 
...
``` r
# Run the bootstrap to create r - squared plot
weather_df %>% 
  bootstrap(1000, id = "strap_number") %>% 
...
![](p8105_hw6_kdh2140_files/figure-gfm/unnamed-chunk-8-1.png)<!-- -->
...
``` r
# Run the bootstrap to create log_beta plot
weather_df %>% 
  bootstrap(1000, id = "strap_number") %>% 
...
![](p8105_hw6_kdh2140_files/figure-gfm/unnamed-chunk-10-1.png)<!-- -->
